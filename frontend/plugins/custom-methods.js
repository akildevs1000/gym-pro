export default ({ app }, inject) => {
  ...
  inject("utils", {
    ...
    currency_format(n, type = "AED") {
      if (isNaN(n) || !n) {
        n = 0;
      }

      // Format number with 2 decimals and commas
      const formattedNumber = parseFloat(n).toLocaleString("en-US", {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2,
      });

      // Define allowed currency codes and fallback default
      const currencies = ["AED", "INR", "USD"];
      const currencyCode = currencies.includes(type.toUpperCase()) ? type.toUpperCase() : "AED";
