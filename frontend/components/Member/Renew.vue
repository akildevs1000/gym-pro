  ...
  <v-dialog persistent v-model="viewDialog" max-width="600px" :key="memberId">
    ...
    <template v-slot:activator="{ on, attrs }">
      <span v-bind="attrs" v-on="on">
        <v-icon color="text" small>mdi-calendar-refresh</v-icon>
        Renew Plan
      </span>
    </template>
    ...
    <v-card flat class="background">
      <v-alert dense class="primary white--text">
        Renew Plan for Member ({{ memberObject.full_name }})
      </v-alert>
      <v-card-text>
        <v-container v-if="memberObject && memberObject.last_membership">
          ...
          <!-- Payment Details -->
          <div
            class="text-subtitle-2 font-weight-medium grey--text text--darken-2 mt-8 mb-2"
          >
            Payment Details
          </div>
          <v-row>
