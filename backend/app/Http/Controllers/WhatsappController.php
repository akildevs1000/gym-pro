...
class WhatsappController extends Controller
{
    ...
    public function sentOTP($data)
    {
        ...
        try {
            if ($data['instance_id']) {
                ...
                Log::channel('custom')->info('Company Insance ID is not exist');
            }
        } catch (\Throwable $th) {
    ...
    public function sendWhatsappNotification($company, $message, $number, $attachments = [])
    {
        ...
        if ($response->successful()) {
            //Log::channel('customer')->info($response->json());
        } else {
            Log::channel('customer')->info($response->body());
        }
    ...
    public function sendMessage($message, $number)
    {
        ...
        // You can check the response status and get the response content as needed
        if ($response->successful()) {
            // Log::channel('custom')->info($response->json());
        } else {
            Log::channel('custom')->info($response->body());
        }
        return $message;
    }
}
