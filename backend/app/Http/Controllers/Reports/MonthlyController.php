...
class MonthlyController extends Controller
{
    ...
    public function monthly(Request $request)
    {
        ...
        $company = [
         ...
         if ($from_date !== $to_date) {
            ...
            return $this->processTemplate3($shift_type, $company);
    ...
    public function processTemplate3($shift_type, $company, $action = "I")
    {
        ...
        $model = Attendance::query();
        $model->whereBetween("date", $range);

        $model->whereHas('member', function ($q) {
            $q->where('status', 1);
        });
       ...
       $count = count($attendances->toArray());
        ...
        foreach ($chunks as $chunk) {
            ...
            $file_path = "pdf/$yesterday/summary_report_$counter.pdf";
            Storage::disk('local')->put($file_path, $data);
        ...
        $filesDirectory = storage_path("app/pdf/$yesterday");
